...
void PR(int* inArrivalTime, int* inBurstTime, int* inPriority, int inN, int isPreemptive) {
    ...
    while (currentTime<=totalCompleteTime) {
        ...
        if (processInRun!=-1) {
            ...
            updatePriority(inPriority, burstTime, num);
        }
...
void SRT(int* inArrivalTime, int* inBurstTime,int inN){
    ...
    PR(inArrivalTime, inBurstTime, priority, inN, 1); // preemptive PR
}
...
int main() {
    ...
    SRT(arrivalTime, burstTime, n);
    printf("CPU Shortest Shortest Remaining Time Scheduling\n");
